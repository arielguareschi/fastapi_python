...
async def post_artigo(artigo: ArtigoSchema, usuario_logado=Depends(get_current_user),
...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, usuario_logado=Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
...
async def delete_artigo(artigo_id: int, usuario_logado=Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    async with db as session:
