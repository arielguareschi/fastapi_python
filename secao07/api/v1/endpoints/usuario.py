...
def post_usuario(usuario: UsuarioSchemaCreate, db = Depends(get_session)):
...
def get_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: Session = Depends(get_session)):
    with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = session.execute(query)
